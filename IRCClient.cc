...
void create_room(char * croom) {
	char response[ MAX_RESPONSE ] = {0};
	response[0] = '\0';

	sendCommand(host, port, "CREATE-ROOM", user, password, croom, response);
...
void enter_room() {
	...
	if (!strcmp(response, "OK\r\n")) {
		message = g_strdup_printf("%s %s",  "entered room:", room);
		send_message();
		// Clear users list
...
void leave_room(char * troom) {
	...
	message = g_strdup_printf("%s %s", "leaves room:", troom);
	send_message();

	sendCommand(host, port, "LEAVE-ROOM", user, password, args, response);
	if (!strcmp(response,"OK\r\n")) {
		printf("User %s leave room:%s\n", user, troom);
		room = (char *)"";
	}
}
